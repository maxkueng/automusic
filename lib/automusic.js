...
mb.lookupCache = function (uri, force, callback, lookup) {
	...
	if (force || !mbLookupQueue.exists(key)) {
	...
	this.discReleaseMappingQueue.add(discId, true, function (data) {
		...
		self.trackMetadata(discId, releaseId, trackNumber, filePath, function (err, metadata) {
			...
			if (!self.releaseArtworkQueue.isBusy(releaseId)) {
				...
				albumart.get(releaseId, config.albumart_dir, false, function (err, imgPath) {
					console.log('aa', err, imgPath);
					self.emit('albumart', err, releaseId);
				});
			}
		});
	});
...
Automusic.prototype.trackMetadata = function (discId, releaseId, trackNumber, filePath, callback) {
	...
	release.load(['release-groups', 'recordings', 'mediums', 'labels', 'artists', 'discids'], function (err) {
		...
		recording.load(['artists', 'work-rels', 'artist-rels'], function (err) {
			...
			process.nextTick(function () {
				self.emit('trackmetadata', releaseId, recording.id);
...
Automusic.prototype.updateRelease = function (releaseId) {
	var release = this.release(releaseId);
	var self = this;
	release.update(function () {
		console.log(release.id);
		self.emit('releaseupdated', releaseId);
	});
};

Automusic.prototype.updateAlbumArt = function (releaseId, uri) {
	var self = this;
	albumart.get(releaseId, config.albumart_dir, uri, true, function (err, imgPath) {
		console.log('aa', err, imgPath);
		self.emit('albumart', err, releaseId);
	});
};
