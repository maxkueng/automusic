...
var get = function (releaseId, destinationPath, force, callback) {
	if (typeof callback === 'undefined') { callback = force; force = false; }
	if (typeof callback === 'undefined') callback = function () {};

	if (!force && path.existsSync(path.join(destinationPath, releaseId + '.jpg'))) {
		callback(false);
		return;
	}
	...
	getImageURI(releaseId, function (err, uri) {
		...
		download(uri, path.join(destinationPath, releaseId), function (err, filePath) {
			im.identify(filePath, function(err, features){
				...
				im.convert([filePath, destinationPath + '.jpg'], function(err, metadata){
					if (err) { callback(err, null); return; }
					callback(false, destinationPath + '.jpg');
				});
			});
		});
	});
